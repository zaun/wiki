<template>
    <div class="section mb-4">
        ...
        <div v-if="!editable">
            ...
            <div v-else-if="contentType === 'flowchart' || contentType === 'state' || contentType ===  'sequence' || contentType ===  'packet'">
                <div><strong>{{ contentType }}</strong></div>
                <div>{{ localSection.data }}</div>
                <SimpleMarkdown v-if="localSection.content" :class="contentClass" :content="localSection.content" />
                <Mermaid :type="contentType" :content="localSection.data" />
                <SimpleMarkdown v-if="localSection.sumary" :class="contentClass" :content="localSection.sumary" />
            </div>

            <div v-else>
                <div v-html="localSection.type"></div>
                <div v-html="localSection.content"></div>
                <div v-html="localSection.data"></div>
                <div v-html="localSection.summary"></div>
            </div>
...
import Mermaid from '@/components/sections/Mermaid.vue';
...
watch(
    ...
    (newType) => {
        ...
        if (newType === props.modelValue.type) {
            localSection.value.data = JSON.parse(JSON.stringify(props.modelValue.data));
            localSection.value.content = props.modelValue.content || '';
            localSection.value.summary = props.modelValue.summary || '';
        }

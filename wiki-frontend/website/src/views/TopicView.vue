...
<template>
    <div>
        <ReviewAlert v-if="!loading && !notFound" :review="aiReviewFlags" />
                ...
                <div class="main-content" ref="sectionList">
                    ...
                    <Section :header-size="4" v-model="node">
                        <template #appendTitle>
                            <ReviewScore :review="aiReviewItems" :ai="true" />
                        </template>
                        <!-- 
                            <Voting />
                        </template> -->
...
const processAIReview = (review, title)  => {
    review.forEach((item) => {
        if (item.hasOwnProperty('result')) {
            if (item.result) {
                aiReviewFlags.value.push({ ...item, title });
            }
        } else {
            aiReviewItems.value.push({ ...item, title });
        }
    });
}
...
async function load() {
    ...
    document.title = `OmniOntos: ${node.value.title}`;

    // Update review items
    aiReviewItems.value = [];
    aiReviewFlags.value = [];
    if (store.currentNode.aiReview) {
        processAIReview(store.currentNode.aiReview,'Summary', true);
    }
    store.currentSections.forEach((section) => {
        if (section.aiReview) {
            processAIReview(section.aiReview, section.title);
        }
    });
}
